          ...
          {categories.map((category) => {
            ...
            return (
              <motion.div 
                ...
                className="relative overflow-hidden rounded-lg transition"
              >
                <Link to={`/shop/${formattedCategory}`} className="block relative">
                  <img src={category.image} alt={category.name} className="rounded-lg w-full h-72 object-cover" />
                  <motion.div className="absolute inset-0 bg-black/40 flex items-center justify-center text-white text-2xl font-semibold transition">
                    {category.name}
                  </motion.div>
                </Link>
              </motion.div>
